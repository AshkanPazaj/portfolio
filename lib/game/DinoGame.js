...
const HIGH_SCORE_KEY = 'ashkan-chrome-dino-hi'
...
export default class DinoGame extends GameRunner {
  ...
  resize(displayWidth, displayHeight) {
    ...
    this.canvas.width = Math.floor(GAME_WIDTH * scale)
    this.canvas.height = Math.floor(GAME_HEIGHT * scale)

    ctx.setTransform(1, 0, 0, 1, 0, 0)
    ctx.scale(scale, scale)

    const { settings, dino } = this.state
    if (dino) {
  ...
  drawScore() {
    ...
    const hiText = `HI ${(this.highScore + '').padStart(5, '0')}`
    const scoreText = (drawValue + '').padStart(5, '0')
    const scoreBlockW = fontSize * 14
    const scoreBlockH = lineHeight * 2

    canvasCtx.fillStyle = '#f7f7f7'
    canvasCtx.fillRect(this.width - scoreBlockW, 0, scoreBlockW, scoreBlockH)

    this.paintText(hiText, this.width, 0, {
      font: 'PressStart2P',
      size: `${fontSize}px`,
      align: 'right',
      baseline: 'top',
    ...
    if (shouldDraw) {
      this.paintText(scoreText, this.width, lineHeight, {
        font: 'PressStart2P',
        size: `${fontSize}px`,
        align: 'right',
        baseline: 'top',
