...
export default class DinoGame extends GameRunner {
  constructor(width, height) {
    ...
    this.highScoreKey = 'chrome-dino-highscore'
  ...
  drawScore() {
    ...
    const fontSize = 12
    const scoreColumnWidth = fontSize * 7
    const scoreAreaHeight = this.highScore > 0 ? fontSize * 2 + 6 : fontSize
    let shouldDraw = true
    ...
    if (this.highScore > 0) {
      this.paintText(
        `HI ${(this.highScore + '').padStart(5, '0')}`,
        this.width,
        fontSize + 6,
        {
          font: 'PressStart2P',
